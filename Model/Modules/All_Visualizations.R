...
# Results module2a --------------------------------------------------------
# Load test results
Farms_result_100 = readRDS("../Data/2a_TestSlaughter/TestSlaughter100%_beta0.05_sample5_sens0.85_2.rds")
Farms_result_50 = readRDS("../Data/2a_TestSlaughter/TestSlaughter50%_beta0.05_sample5_sens0.85_2.rds")
Farms_result_33 = readRDS("../Data/2a_TestSlaughter/TestSlaughter33%_beta0.05_sample5_sens0.85_2.rds")

# Set non detect to 1500 days 
Farms_result_100$discover_days[Farms_result_100$discover_days > 1094] = 2000
Farms_result_50$discover_days[Farms_result_50$discover_days > 1094] = 2000
Farms_result_33$discover_days[Farms_result_33$discover_days > 1094] = 2000

# Set non detect to 250 propagation
Farms_result_100$Propagation[Farms_result_100$discover_days > 1094] = 250
Farms_result_50$Propagation[Farms_result_50$discover_days > 1094] = 250
Farms_result_33$Propagation[Farms_result_33$discover_days > 1094] = 250
...
# Plot the number of infected herds at detection for 100%, 50% and 33% of movements
p_prop_all = ggplot(Farms_result_all, aes(x = m_freq, y=Propagation, fill = m_freq)) + 
...
p_days_all = ggplot(Farms_result_all, aes(x = m_freq, y = discover_days, fill = m_freq)) + 
  geom_half_violin(side = "r", alpha = 1) +
  geom_half_boxplot(data = Farms_result_all %>% filter(discover_days < 1094),
...
plot_grid(p_prop_all, p_days_all, labels="AUTO")
...
Farms_result_50sample_vir = readRDS("../Data/2a_TestSlaughter/TestSlaughter100%_beta0.2_sample50_sens0.7_2.rds")
...
result_testType = plot_grid(p_prop_testType, p_days_testType, labels="AUTO")
#Show both plots
...
# For sjov bacteria
Visual_results_bac = readRDS("../Data/2b_VisualInspection/Detect_bac.rds")
...
monthly_sendings = ggplot(pigs_sent_per_month, aes(x = month, y = Total_pigs_sent)) +
  ...
  labs(title = "Total Number of Pigs Sent Each Month",
       ...
       y = "Pigs Sent [#]") 
# Save plot
ggsave("../Results/Data/MonthlySendings.tiff", units="cm", width=18, height=8, dpi=300, compression = 'lzw')

# plot an save monthly and weekly together
month_week_Sendings = arrangeGrob(weekly_sending, monthly_sendings,
                                  ncol = 1, nrow = 2)
ggsave("../Results/Data/month_week_Sendings.tiff", plot = month_week_Sendings, units="cm", width=18, height=16, dpi=300, compression = 'lzw')
...
ggplot(data=dist1, aes(x=RECEIVER_FARMTYPE, y=Npigs, fill=RECEIVER_FARMTYPE)) +
  ...
  facet_wrap(~SENDER_FARMTYPE, scales="free_y", ncol = 3, 
  ...
  scale_fill_manual(name = "Receiving Herd type",
                    values = c("#FFFFCC", "#C7E9B4" ,"#7FCDBB", "#41B6C4", "#2C7FB8", "#253494"),
...
Sent_plot <- ggplot(pigs_sent_per_week, aes(x = week, y = Total_pigs_sent, color = SENDER_FARMTYPE, group = SENDER_FARMTYPE)) +
  ...
  geom_point() +
  scale_y_log10() +
  labs(title = "Weekly sent pigs by each Farm Type",
       x = "Date",
       y = "Pigs Sent [#]",
       color = "Farm Type") +
...
# compute number of pigs sent monthly across the farm types 
pigs_receive_per_month <- movement.data %>%
...
Receive_plot_month <- ggplot(pigs_receive_per_month, aes(x = month, y = Total_pigs_receive, color = RECEIVER_FARMTYPE, group = RECEIVER_FARMTYPE)) +
  ...
  geom_point() +
  scale_y_log10() +
  labs(title = "Monthly Receivings of pigs by each Farm Type",
       x = "Time [Month]",
       y = "# Pigs Received",
       color = "Farm Type") +
  scale_color_manual(name = "Herd type",
...
overshootTwoPlots = arrangeGrob(overshoot_send, overshoot_receive,
                                overshoot_send_year,overshoot_receive_year,
                                ncol = 2, nrow = 2)
