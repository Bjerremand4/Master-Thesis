...
transmission.data = readRDS(file.path(dirname(getwd()), paste0("Data/1_Transmission/trans_move",move_freq,"%_beta", agent_type, ".rds")))
...
if ( move_freq_default == T & beta_default == T & test_default == T){
  Farms_result = readRDS(file.path(dirname(getwd()),paste0("Data/2a_TestSlaughter/TestSlaughter",move_freq, "%_beta", agent_type,"_sample", n_test, "_sens", test_sensitivity, ".rds")))
} else {
...
if (pathogen == T){
  if(move_freq_default == T & beta_default == T){
    Detection_result = readRDS(file.path(dirname(getwd()),paste0("Data/2b_VisualInspection/Detect_beta", agent_type, "move_",move_freq,"%.rds")))
  } else {
