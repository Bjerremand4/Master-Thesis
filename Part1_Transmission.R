...
herdID = setdiff(farmIDs, slaught_ID)[1:10]
...
for (herd in 1:length(herdID)){
  ...
  farms_infected = data.frame(farmID = init_farm,
                              ...
                              date_of_infection = move.data$DATE_MOVEMENT[1],
                              farmID_spreader = NA,
                              init_inf = 1,
                              I = 1,
  ...
  # Initialize time 
  start.date <- move.data$DATE_MOVEMENT[1]            
  ...
  while (is.na(next.event.date) != T){
    ...
    result <- Logistic_model(N = farms_infected$Capacity, I0 = farms_infected$I, t = days, beta = bac_beta)
    ...
    moves = move.data %>% filter(FARMID_SENDER %in% farms_infected$farmID & DATE_MOVEMENT == current.event.date)
    ...
    for (move in 1:nrow(moves)){
      ...
      if (n_spread > 0){
        ...
        if (!(farmID_reciver %in% farms_infected$farmID) & receiver_type != "15 68 Svineslagteri") {
    ...
    next.event.date = next.event.date = move.data %>%
      filter(FARMID_SENDER %in% farms_infected$farmID) %>% 
      filter(DATE_MOVEMENT > current.event.date) %>%
      pull(DATE_MOVEMENT) %>% first()
...
#saveRDS(Output_list, file.path("../Data", "full_trans_beta005.rds") )
