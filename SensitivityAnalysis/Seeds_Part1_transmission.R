...
# saveRDS(seeds, file.path("../Data", "seeds10.rds"))
...
for (seed in seeds){
  ...
  for (herd in 1:length(herdID)){
    ...
    while (is.na(next.event.date) != T){
      ...
      result <- Logistic_model(N = farms_infected$Capacity, I0 = farms_infected$I, t = days, beta = 0.05)
    ...
    print(paste("run seed", seed_nr, "iteration", herd))
  }
...
#saveRDS(seed_output_list, file.path("../Data", "transmission_10seeds.rds") )
